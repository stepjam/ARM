...
class UniformReplayBuffer(ReplayBuffer):
    ...
    - invalid_range: np.array, an array with the indices of cursor-related 
                    invalid transitions
    '''
    def __init__(self,
                 batch_size: int = 32,
        ...
        self._action_shape, self._action_dtype = action_shape, action_dtype
        self._reward_shape, self._reward_dtype = reward_shape, reward_dtype
    ...
    def get_storage_signature(
            self) -> Tuple[List[ReplayElement], List[ReplayElement]]:
        '''
        ...
        for obs_element in self._observation_elements:
            obs_elements.append(
                ReplayElement(obs_element.name, obs_element.shape,
                              obs_element.type))
    ...
    def add(self, action, reward, terminal, timeout, **kwargs):
        ...
        - terminal: A uint8 acting as a boolean indicating whether 
                    the transition was terminal (1) or not (0).
        - **kwargs: The remaining args
        '''

        # If previous transition was a terminal, then add_final wasn't called
        if not self.is_empty() and self._store['terminal'][self.cursor() -
                                                           1] == 1:
    ...
    def _add_initial_to_disk(self, kwargs: dict):
        for i in range(self._timesteps - 1):
            with open(
                    join(self._save_dir,
                         '%d.replay' % (self._replay_capacity - 1 - i)),
                    'wb') as f:
    ...
    def _add(self, kwargs: dict):
        ...
        with self._lock:
            ...
            if self._disk_saving:
                ...
                with open(join(self._save_dir, '%d.replay' % cursor),
                          'wb') as f:
                    pickle.dump(kwargs, f)
    ...
    def get_range(self, array, start_index, end_index):
        '''
        Returns the range of array at the index handling wraparound if 
        necessary.

        Input:
        - array (np.array): the array to get the stack from.
        - start_index (int): index to the start of the range to be 
                             returned. Range will wraparound if start_index 
                             is smaller than 0.
        - end_index (int): exclusive end index. Range will wraparound if 
                           end_index exceeds replay_capacity.

        Output:
        - return_array (np.array): with shape [end_index - start_index, 
                                   array.shape[1:]].
        '''
    ...
    def get_range_stack(self, array, start_index, end_index, terminals=None):
        ...
        if terminals is None:
            terminals = self.get_range(self._store[TERMINAL], start_index,
                                       end_index)
    ...
    def sample_index_batch(self, batch_size):
        ...
        if self.is_full():
            ...
            min_id = (self.cursor() - self._replay_capacity + self._timesteps -
                      1)
    ...
    def unpack_transition(self, transition_tensors, transition_type):
        '''
        Unpacks the given transition into member variables. To be more,
        specific, we reform the transition into `collections.OrderedDict`
    ...
    def sample_transition_batch(self,
        ...
        with self._lock:
            ...
            # get the name of the transition elements
            # print('replay_buffer batch_size', batch_size)
            transition_elements = self.get_transition_elements(batch_size)
            ...
            for batch_element, state_index in enumerate(indices):
                ...
                # calculate the index of the next state according to
                # self._update_horizon and termination of the trajectory
                ...
                if not is_terminal_transition:
                    ...
                    trajectory_length = np.argmax(
                        trajectory_terminals.astype(np.bool), 0) + 1
        ...
        if pack_in_dict:
            batch_arrays = self.unpack_transition(batch_arrays,
                                                  transition_elements)
        return batch_arrays
    ...
    def get_transition_elements(self, batch_size=None):
        ...
        To be more specific, we first pack basic transition information, then,
        we pack `self._observation_elements` (with '_tp1') and 
        `self._extra_replay_elements`.
        ...
        for element in self._observation_elements:
            transition_elements.append(
                ReplayElement(element.name, (batch_size, self._timesteps) +
                              tuple(element.shape), element.type, True))
            transition_elements.append(
                ReplayElement(element.name + '_tp1',
                              (batch_size, self._timesteps) +
                              tuple(element.shape), element.type, True))
