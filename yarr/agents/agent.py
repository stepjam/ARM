...
class ActResult(object):
    def __init__(self,
                 ...
                 info: dict = None):
        '''
        pack the action and related results

        Input:
        - action: the action predicted by the agent
        - observation_elements (dict): the processed observation used in 
                                       downstream framework 
        - replay_elements (dict)
        - info (dict): remaining information of a transition
        '''
